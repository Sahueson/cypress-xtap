describe('Login, navegar a Empresas y añadir una empresa', () => {
    ...
    const randomString = Math.random().toString(36).substring(2, 8); // Genera una cadena alfanumérica
    const emailUser = `cypress_${randomString}@example.com`;
    ...
    it('Revisar que se creo la empresa', () => {       
        ...
        const contacto = nombreUser + ' ' + apellidoUser;
        ...
        // Verifica que estan los campos
        cy.get('table tbody tr').first()
        .find('td').eq(0)
        .should('contain.text', nombreEmpresa);
